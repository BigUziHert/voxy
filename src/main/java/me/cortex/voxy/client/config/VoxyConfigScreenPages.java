    ...
    public static OptionPage page() {
        ...
        groups.add(OptionGroup.createBuilder()
                .add(OptionImpl.createBuilder(boolean.class, storage)
                    ...
                    .setTooltip(Component.literal("Multiplier for terrain fog density. 0 = off, 100 = vanilla, 200 = double"))
                    .setControl(opt -> new SliderControl(opt, 0, 2000, 10, v -> Component.literal(String.format("%.1f", v / 100.0f))))
                    .setBinding((s, v) -> s.fogIntensity = v / 100.0f, s -> (int)(s.fogIntensity * 100))
                    .build()
                ).add(OptionImpl.createBuilder(int.class, storage)
                    .setName(Component.literal("Fog curve"))
                    .setTooltip(Component.literal("Exponential fog curve. 0 = linear, higher = fog builds up near the end"))
